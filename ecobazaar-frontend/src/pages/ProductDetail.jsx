...
const ProductDetail = () => {
  ...
  const loadProduct = async () => {
    try {
      ...
      setProduct(res.data);
      if (isAuthenticated) checkWishlistStatus(id);
    } catch {
  ...
  const checkWishlistStatus = async (productId) => {
    try {
      const res = await customerAPI.checkWishlistStatus(productId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white">
      <div className="max-w-7xl mx-auto px-6 py-10">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center text-emerald-700 mb-8 hover:text-emerald-900"
        >
          <ArrowLeft size={20} className="mr-2" />
          Back
        </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14">
          ...
          >
            ...
            <button
              onClick={handleWishlist}
              className={`absolute top-5 right-5 p-3 rounded-full backdrop-blur bg-white/80 shadow ${
          ...
          {/* Info */}
          <div>
            <span className="inline-flex items-center px-4 py-1 rounded-full bg-emerald-100 text-emerald-800 text-sm font-medium mb-4">
              <Leaf size={14} className="mr-1" />
              {product.isEcoFriendly ? 'Eco-Friendly' : 'Standard'}
            </span>
            ...
            <div className="flex items-center mb-6">
              ...
              <span className="ml-3 text-gray-600">(4.5 · 128 reviews)</span>
            </div>

            <div className="bg-gradient-to-r from-emerald-600 to-green-500 text-white p-6 rounded-2xl mb-6 shadow-lg">
            ...
            <div className="flex gap-4 mt-8">
              <button
                onClick={handleAddToCart}
                className="flex-1 py-4 rounded-full bg-emerald-600 text-white font-semibold hover:bg-emerald-700 transition flex items-center justify-center"
              >
            ...
            <div className="grid grid-cols-2 gap-4 mt-10">
              {[Truck, Shield, Award, Leaf].map((Icon, i) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <span className="font-medium text-gray-700">
                    {['Free Shipping', 'Quality Guarantee', 'Eco Certified', 'Carbon Neutral'][i]}
                  </span>
                </div>
              ))}
            </div>
